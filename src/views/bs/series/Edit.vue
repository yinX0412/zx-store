<template>
    <div v-loading="loading">
        <div style="padding-bottom: 10px;border-bottom: 1px solid #ddd;margin-bottom: 20px">
            <el-button @click="$router.go(-1)">返回</el-button>
        </div>
        <el-form ref="form" :model="form">
            <el-form-item label="系列名称" required>
                <el-input v-model="form.series_name" size="small" placeholder="请输入系列名称"
                          style="width: 200px"></el-input>
            </el-form-item>
            <el-form-item label="系列图片" required>
                <el-image v-if="form.image" :src="form.image"
                          style="width: 200px;vertical-align: middle"></el-image>
                <el-upload
                    style="display: inline-block;vertical-align: middle;margin-left: 10px;"
                    action=""
                    accept="image/*"
                    :auto-upload="false"
                    :on-change="uploadImg"
                    :show-file-list="false">
                    <el-button size="small" type="primary">点击上传</el-button>
                </el-upload>
            </el-form-item>
        </el-form>
        <el-button type="primary" size="small" @click="saveSeries">保存</el-button>
    </div>
</template>
<script lang="ts">
    import {Vue, Component} from 'vue-property-decorator';
    import {ElUploadInternalFileDetail} from 'element-ui/types/upload';
    import {uploadImg} from '@/service/bs/bs-img-upload-service';
    import {createSeries, getSeries} from '@/service/bs/bs-series-service';
    import {updateSeries} from '../../../service/bs/bs-series-service';

    @Component
    export default class BsCreateSeries extends Vue {
        private loading: boolean = false;
        private form: { id: string, series_name: string, image: string } = {
            series_name: '',
            image: '',
            id: '',
        };

        private created(): void {
            this.getSeries();
        }

        private getSeries() {
            this.loading = true;
            getSeries((this.$route.params as any).id)
                .then((res) => {
                    this.form = {
                        id: res.id,
                        series_name: res.series_name,
                        image: this.baseUrl + res.image,
                    };
                })
                .catch((error) => this.$message({message: error, type: 'warning'}))
                .finally(() => this.loading = false);
        }

        private uploadImg(file: ElUploadInternalFileDetail) {
            if (file.size > 1024 * 1024 * 10) {
                this.$message({
                    message: '请上传大小10M以内的图片',
                    type: 'warning',
                });
                return;
            }
            this.loading = true;
            uploadImg(file.raw)
                .then((res) => {
                    this.form.image = this.baseUrl + res.path;
                })
                .catch((error) => this.$message({message: error, type: 'warning'}))
                .finally(() => this.loading = false);
        }

        private saveSeries() {
            if (!this.form.series_name) {
                this.$message({
                    message: '请输入系列名称',
                    type: 'warning',
                });
                return;
            }
            if (!this.form.image) {
                this.$message({
                    message: '请选择系列图片',
                    type: 'warning',
                });
                return;
            }
            this.loading = true;
            updateSeries(this.form.id, this.form.series_name, this.form.image)
                .then(() => {
                    this.$message({
                        message: '保存成功',
                        type: 'success',
                    });
                    this.$router.replace('/bs/series/list');
                })
                .catch((error) => this.$message({message: error, type: 'warning'}))
                .finally(() => this.loading = false);
        }
    }
</script>
