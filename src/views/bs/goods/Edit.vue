...
<template>
    <div v-loading="loading">
        ...
        <el-button type="primary" size="small" @click="updateGoods">保存</el-button>
        <el-dialog
            title="分类"
        ...
        <el-dialog
            ...
            <span slot="footer" class="dialog-footer">
            <el-button @click="skuDetailDialog=false">取 消</el-button>
...
<script lang="ts">
    ...
    import {getGoods, updateGoods, UpdateGoodsArgs} from '../../../service/bs/bs-goods-service';

    const E = require('wangeditor');

    @Component
    export default class BsUpdateGoods extends Vue {
        private loading: boolean = false;
        private categoryDialog: boolean = false;
        private seriesDialog: boolean = false;
        private skuDetailDialog: boolean = false;
        private form: UpdateGoodsArgs = {
            id: '',
            goods_name: '',
            main_img: '',
            video_img: '',
        ...
        private getGoods() {
            getGoods((this.$route.params as any).id)
                .then((res) => {
                    let sku = res.goods_sku;
                    sku.forEach((item) => {
                        item.sku_content = JSON.parse(item.sku_content);
                    });
                    console.log(sku);
                    this.form = {
                        id: res.id,
                        ...
                        normal_price: res.normal_price,
                        video_price: res.video_price,
                    ...
                    this.currentSeries = res.series;
                    this.richtext.txt.html(res.goods_detail);
                });
        }
